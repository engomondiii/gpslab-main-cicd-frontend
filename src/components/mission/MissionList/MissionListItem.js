 ...
 * GPS 101 INTEGRATION: Displays GPS 101 badge, links to GPS 101 pages, shows GPS 101 progress
 */
...
const MISSION_STATUS = {
  ...
  available: {
    label: 'Available',
    icon: <svg viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-8.707l-3-3a1 1 0 00-1.414 1.414L10.586 9H7a1 1 0 100 2h3.586l-1.293 1.293a1 1 0 101.414 1.414l3-3a1 1 0 000-1.414z" clipRule="evenodd"/></svg>,
    color: 'available'
  },
  in_progress: {
    label: 'In Progress',
  ...
  failed: {
    label: 'Failed',
    icon: <svg viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clipRule="evenodd"/></svg>,
    color: 'failed'
  }
};
...
const MISSION_TYPE_ICONS = {
  ...
  gps101: <svg viewBox="0 0 24 24" fill="currentColor"><path d="M10.394 2.08a1 1 0 00-.788 0l-7 3a1 1 0 000 1.84L5.25 8.051a.999.999 0 01.356-.257l4-1.714a1 1 0 11.788 1.838L7.667 9.088l1.94.831a1 1 0 00.787 0l7-3a1 1 0 000-1.838l-7-3zM3.31 9.397L5 10.12v4.102a8.969 8.969 0 00-1.05-.174 1 1 0 01-.89-.89 11.115 11.115 0 01.25-3.762zM9.3 16.573A9.026 9.026 0 007 14.935v-3.957l1.818.78a3 3 0 002.364 0l5.508-2.361a11.026 11.026 0 01.25 3.762 1 1 0 01-.89.89 8.968 8.968 0 00-5.35 2.524 1 1 0 01-1.4 0zM6 18a1 1 0 001-1v-2.065a8.935 8.935 0 00-2-.712V17a1 1 0 001 1z"/></svg>,
...
const MissionListItem = ({
  ...
  gps101StageNumber, // NEW: GPS 101 stage number (1-5)
  gps101MissionNumber, // NEW: GPS 101 mission number within stage
  className = '',
  ...props
}) => {
  ...
  const TypeIcon = MISSION_TYPE_ICONS[isGPS101 ? 'gps101' : type] || MISSION_TYPE_ICONS.standard;
  ...
  const classNames = [
    ...
    `mission-list-item--${size}`,
    `mission-list-item--${variant}`,
    isGPS101 && 'mission-list-item--gps101', // NEW: GPS 101 class
    onClick && !isLocked && 'mission-list-item--clickable',
    className
  ].filter(Boolean).join(' ');
  
  // NEW: GPS 101-aware click handler
  const handleClick = () => {
    if (isLocked) return;
    
    if (isGPS101) {
      // Route to GPS 101-specific mission page
      navigate(`/gps-101/stage/${gps101StageNumber || stageNumber}/mission/${gps101MissionNumber || missionNumber || id}`);
    } else if (onClick) {
      onClick(id);
    }
  };
  ...
  return (
    <div
      ...
      {...props}
    >
      {/* NEW: GPS 101 Badge Overlay */}
      {isGPS101 && (
        <div className="mission-list-item__gps101-badge">
      ...
      <div className={`mission-list-item__type mission-list-item__type--${isGPS101 ? 'gps101' : type}`}>
        {TypeIcon}
        {isPartyMission && (
          <span className="mission-list-item__party-badge">{partySize || '2-8'}</span>
        )}
      </div>
      ...
      <div className="mission-list-item__content">
        <div className="mission-list-item__header">
          <div className="mission-list-item__title-row">
            {/* NEW: GPS 101 mission numbering */}
            {missionNumber && (
              <span className="mission-list-item__number">
                {isGPS101 && gps101StageNumber && `Stage ${gps101StageNumber} • `}
                {stageNumber && !isGPS101 && `S${stageNumber}.`}
                M{gps101MissionNumber || missionNumber}
              </span>
            )}
        ...
        {isInProgress && (
          <div className="mission-list-item__progress">
            <div className="mission-list-item__progress-bar">
              <div
                className="mission-list-item__progress-fill"
                style={{ width: `${progress}%` }}
              />
            </div>
            <span className="mission-list-item__progress-text">
              {progress}% • {completedObjectives}/{objectives.length} objectives
            </span>
          </div>
        )}
        ...
        <div className="mission-list-item__meta">
          ...
          {estimatedTime && (
            <div className="mission-list-item__time">
              <svg viewBox="0 0 20 20" fill="currentColor">
          ...
          {tags.length > 0 && (
            <div className="mission-list-item__tags">
              {tags.slice(0, 3).map(tag => (
                <span key={tag} className="mission-list-item__tag">{tag}</span>
              ))}
            </div>
          )}
        </div>
      ...
      <div className="mission-list-item__actions">
        {!isLocked && (xpReward > 0 || barakaReward > 0) && (
          <div className="mission-list-item__rewards">
            {xpReward > 0 && (
              <span className="mission-list-item__reward mission-list-item__reward--xp">
                +{xpReward} XP
              </span>
            )}
            {barakaReward > 0 && (
              <span className="mission-list-item__reward mission-list-item__reward--baraka">
                +{barakaReward} ƀ
              </span>
            )}
          </div>
        ...
        {status === 'available' && onAccept && (
          <button
            ...
            className="mission-list-item__accept-btn"
          >
            Accept
            <svg viewBox="0 0 20 20" fill="currentColor">
        ...
        {isInProgress && (
          <button
            type="button"
            onClick={handleClick}
            className="mission-list-item__continue-btn"
          >
            Continue
            <svg viewBox="0 0 20 20" fill="currentColor">
        ...
        {isCompleted && (
          <div className="mission-list-item__completed-badge">
            <svg viewBox="0 0 20 20" fill="currentColor">
        ...
        {isLocked && (
          <div className="mission-list-item__locked-badge">
            <svg viewBox="0 0 20 20" fill="currentColor">
